; -------------------------------------------------------------------------------
; Sprite mappings - Metropolis giant cog teeth
; -------------------------------------------------------------------------------

Map_CogTeeth:	index offset(*)
	ptr	Map_obj70_0040
	ptr	Map_obj70_0052
	ptr	Map_obj70_005C
	ptr	Map_obj70_0066
	ptr	Map_obj70_0070
	ptr	Map_obj70_007A
	ptr	Map_obj70_0084
	ptr	Map_obj70_008E
	ptr	Map_obj70_0098
	ptr	Map_obj70_00AA
	ptr	Map_obj70_00B4
	ptr	Map_obj70_00BE
	ptr	Map_obj70_00C8
	ptr	Map_obj70_00D2
	ptr	Map_obj70_00DC
	ptr	Map_obj70_00E6
	ptr	Map_obj70_00F0
	ptr	Map_obj70_0102
	ptr	Map_obj70_010C
	ptr	Map_obj70_0116
	ptr	Map_obj70_0120
	ptr	Map_obj70_012A
	ptr	Map_obj70_0134
	ptr	Map_obj70_013E
	ptr	Map_obj70_0148
	ptr	Map_obj70_015A
	ptr	Map_obj70_0164
	ptr	Map_obj70_016E
	ptr	Map_obj70_0178
	ptr	Map_obj70_0182
	ptr	Map_obj70_018C
	ptr	Map_obj70_0196

Map_obj70_0040:	spritemap
	piece	-$10, -$10, 2, 4, 0, 0, 0, 0, 0
	piece	0, -$10, 2, 4, 0, 1, 0, 0, 0
	endsprite

Map_obj70_0052:	spritemap
	piece	-$C, -$10, 3, 4, 8, 1, 0, 0, 0
	endsprite

Map_obj70_005C:	spritemap
	piece	-$10, -$10, 4, 4, $14, 1, 0, 0, 0
	endsprite

Map_obj70_0066:	spritemap
	piece	-$10, -$10, 4, 4, $24, 1, 0, 0, 0
	endsprite

Map_obj70_0070:	spritemap
	piece	-$10, -$10, 4, 4, $34, 1, 0, 0, 0
	endsprite

Map_obj70_007A:	spritemap
	piece	-$10, -$10, 4, 4, $44, 1, 0, 0, 0
	endsprite

Map_obj70_0084:	spritemap
	piece	-$10, -$10, 4, 4, $54, 1, 0, 0, 0
	endsprite

Map_obj70_008E:	spritemap
	piece	-$10, -$C, 4, 3, $64, 1, 0, 0, 0
	endsprite

Map_obj70_0098:	spritemap
	piece	-$10, -$10, 4, 2, $70, 1, 0, 0, 0
	piece	-$10, 0, 4, 2, $70, 1, 1, 0, 0
	endsprite

Map_obj70_00AA:	spritemap
	piece	-$10, -$C, 4, 3, $64, 1, 1, 0, 0
	endsprite

Map_obj70_00B4:	spritemap
	piece	-$10, -$10, 4, 4, $54, 1, 1, 0, 0
	endsprite

Map_obj70_00BE:	spritemap
	piece	-$10, -$10, 4, 4, $44, 1, 1, 0, 0
	endsprite

Map_obj70_00C8:	spritemap
	piece	-$10, -$10, 4, 4, $34, 1, 1, 0, 0
	endsprite

Map_obj70_00D2:	spritemap
	piece	-$10, -$10, 4, 4, $24, 1, 1, 0, 0
	endsprite

Map_obj70_00DC:	spritemap
	piece	-$10, -$10, 4, 4, $14, 1, 1, 0, 0
	endsprite

Map_obj70_00E6:	spritemap
	piece	-$C, -$10, 3, 4, 8, 1, 1, 0, 0
	endsprite

Map_obj70_00F0:	spritemap
	piece	-$10, -$10, 2, 4, 0, 0, 1, 0, 0
	piece	0, -$10, 2, 4, 0, 1, 1, 0, 0
	endsprite

Map_obj70_0102:	spritemap
	piece	-$C, -$10, 3, 4, 8, 0, 1, 0, 0
	endsprite

Map_obj70_010C:	spritemap
	piece	-$10, -$10, 4, 4, $14, 0, 1, 0, 0
	endsprite

Map_obj70_0116:	spritemap
	piece	-$10, -$10, 4, 4, $24, 0, 1, 0, 0
	endsprite

Map_obj70_0120:	spritemap
	piece	-$10, -$10, 4, 4, $34, 0, 1, 0, 0
	endsprite

Map_obj70_012A:	spritemap
	piece	-$10, -$10, 4, 4, $44, 0, 1, 0, 0
	endsprite

Map_obj70_0134:	spritemap
	piece	-$10, -$10, 4, 4, $54, 0, 1, 0, 0
	endsprite

Map_obj70_013E:	spritemap
	piece	-$10, -$C, 4, 3, $64, 0, 1, 0, 0
	endsprite

Map_obj70_0148:	spritemap
	piece	-$10, -$10, 4, 2, $70, 0, 0, 0, 0
	piece	-$10, 0, 4, 2, $70, 0, 1, 0, 0
	endsprite

Map_obj70_015A:	spritemap
	piece	-$10, -$C, 4, 3, $64, 0, 0, 0, 0
	endsprite

Map_obj70_0164:	spritemap
	piece	-$10, -$10, 4, 4, $54, 0, 0, 0, 0
	endsprite

Map_obj70_016E:	spritemap
	piece	-$10, -$10, 4, 4, $44, 0, 0, 0, 0
	endsprite

Map_obj70_0178:	spritemap
	piece	-$10, -$10, 4, 4, $34, 0, 0, 0, 0
	endsprite

Map_obj70_0182:	spritemap
	piece	-$10, -$10, 4, 4, $24, 0, 0, 0, 0
	endsprite

Map_obj70_018C:	spritemap
	piece	-$10, -$10, 4, 4, $14, 0, 0, 0, 0
	endsprite

Map_obj70_0196:	spritemap
	piece	-$C, -$10, 3, 4, 8, 0, 0, 0, 0
	endsprite

	even
