Map_objAF_b:	index offset(*)
	ptr	Map_objAF_b_0010
	ptr	Map_objAF_b_003A
	ptr	Map_objAF_b_0064
	ptr	Map_objAF_b_008E
	ptr	Map_objAF_b_00C8
	ptr	Map_objAF_b_0102
	ptr	Map_objAF_b_011C
	ptr	Map_objAF_b_0136

Map_objAF_b_0010:	spritemap
	piece	-$10, -$C, 2, 3, 0, 0, 0, 1, 0
	piece	0, -$C, 2, 3, 0, 1, 0, 1, 0
	piece	-$10, 4, 2, 1, 6, 0, 0, 1, 0
	piece	0, 4, 2, 1, 6, 0, 0, 1, 0
	piece	-$10, -8, 4, 2, $190, 0, 0, 0, 0
	endsprite

Map_objAF_b_003A:	spritemap
	piece	-$10, -$C, 2, 3, 0, 0, 0, 1, 0
	piece	0, -$C, 2, 3, 0, 1, 0, 1, 0
	piece	-$10, 0, 2, 1, 6, 0, 0, 1, 0
	piece	0, 0, 2, 1, 6, 0, 0, 1, 0
	piece	-$10, -8, 4, 2, $190, 0, 0, 0, 0
	endsprite

Map_objAF_b_0064:	spritemap
	piece	-$10, -$C, 2, 3, 0, 0, 0, 1, 0
	piece	0, -$C, 2, 3, 0, 1, 0, 1, 0
	piece	-$10, -4, 2, 1, 6, 0, 0, 1, 0
	piece	0, -4, 2, 1, 6, 0, 0, 1, 0
	piece	-$10, -8, 4, 2, $190, 0, 0, 0, 0
	endsprite

Map_objAF_b_008E:	spritemap
	piece	-$10, -$C, 2, 3, 0, 0, 0, 1, 0
	piece	0, -$C, 2, 3, 0, 1, 0, 1, 0
	piece	-$10, -8, 2, 1, 6, 0, 0, 1, 0
	piece	0, -8, 2, 1, 6, 0, 0, 1, 0
	piece	-$10, 0, 2, 1, 6, 0, 0, 1, 0
	piece	0, 0, 2, 1, 6, 0, 0, 1, 0
	piece	-$10, -8, 4, 2, $190, 0, 0, 0, 0
	endsprite

Map_objAF_b_00C8:	spritemap
	piece	-$10, -$C, 2, 3, 0, 0, 0, 1, 0
	piece	0, -$C, 2, 3, 0, 1, 0, 1, 0
	piece	-$10, -$C, 2, 1, 6, 0, 0, 1, 0
	piece	0, -$C, 2, 1, 6, 0, 0, 1, 0
	piece	-$10, -4, 2, 1, 6, 0, 0, 1, 0
	piece	0, -4, 2, 1, 6, 0, 0, 1, 0
	piece	-$10, -8, 4, 2, $190, 0, 0, 0, 0
	endsprite

Map_objAF_b_0102:	spritemap
	piece	-$10, -$C, 2, 3, 0, 0, 0, 1, 0
	piece	0, -$C, 2, 3, 0, 1, 0, 1, 0
	piece	-$10, -8, 4, 2, $190, 0, 0, 0, 0
	endsprite

Map_objAF_b_011C:	spritemap
	piece	-$10, -$C, 2, 3, 0, 0, 0, 1, 0
	piece	0, -$C, 2, 3, 0, 1, 0, 1, 0
	piece	-$10, -7, 4, 2, $190, 0, 0, 0, 0
	endsprite

Map_objAF_b_0136:	spritemap
	piece	-$10, -$C, 2, 3, 0, 0, 0, 1, 0
	piece	0, -$C, 2, 3, 0, 1, 0, 1, 0
	piece	-$10, -8, 4, 2, $198, 0, 0, 0, 0
	endsprite

	even
