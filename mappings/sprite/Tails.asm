Map_Tails:	index offset(*)
	ptr	Map_Tails_0116
	ptr	Map_Tails_0118
	ptr	Map_Tails_012A
	ptr	Map_Tails_013C
	ptr	Map_Tails_014E
	ptr	Map_Tails_0160
	ptr	Map_Tails_016A
	ptr	Map_Tails_0174
	ptr	Map_Tails_0186
	ptr	Map_Tails_0198
	ptr	Map_Tails_01A2
	ptr	Map_Tails_01AC
	ptr	Map_Tails_01B6
	ptr	Map_Tails_01C0
	ptr	Map_Tails_01CA
	ptr	Map_Tails_01E4
	ptr	Map_Tails_01FE
	ptr	Map_Tails_0218
	ptr	Map_Tails_0232
	ptr	Map_Tails_024C
	ptr	Map_Tails_0266
	ptr	Map_Tails_0280
	ptr	Map_Tails_029A
	ptr	Map_Tails_02B4
	ptr	Map_Tails_02CE
	ptr	Map_Tails_02E8
	ptr	Map_Tails_0302
	ptr	Map_Tails_031C
	ptr	Map_Tails_0336
	ptr	Map_Tails_0350
	ptr	Map_Tails_036A
	ptr	Map_Tails_0384
	ptr	Map_Tails_039E
	ptr	Map_Tails_03B8
	ptr	Map_Tails_03D2
	ptr	Map_Tails_03EC
	ptr	Map_Tails_0406
	ptr	Map_Tails_0420
	ptr	Map_Tails_043A
	ptr	Map_Tails_044C
	ptr	Map_Tails_0466
	ptr	Map_Tails_0478
	ptr	Map_Tails_0492
	ptr	Map_Tails_04A4
	ptr	Map_Tails_04BE
	ptr	Map_Tails_04D0
	ptr	Map_Tails_04EA
	ptr	Map_Tails_04FC
	ptr	Map_Tails_050E
	ptr	Map_Tails_0520
	ptr	Map_Tails_0532
	ptr	Map_Tails_0544
	ptr	Map_Tails_0556
	ptr	Map_Tails_0570
	ptr	Map_Tails_058A
	ptr	Map_Tails_05A4
	ptr	Map_Tails_05BE
	ptr	Map_Tails_05D8
	ptr	Map_Tails_05F2
	ptr	Map_Tails_060C
	ptr	Map_Tails_0626
	ptr	Map_Tails_060C
	ptr	Map_Tails_0640
	ptr	Map_Tails_065A
	ptr	Map_Tails_0674
	ptr	Map_Tails_068E
	ptr	Map_Tails_06A8
	ptr	Map_Tails_06C2
	ptr	Map_Tails_06DC
	ptr	Map_Tails_06F6
	ptr	Map_Tails_0710
	ptr	Map_Tails_071A
	ptr	Map_Tails_0724
	ptr	Map_Tails_072E
	ptr	Map_Tails_0738
	ptr	Map_Tails_0742
	ptr	Map_Tails_074C
	ptr	Map_Tails_0756
	ptr	Map_Tails_0760
	ptr	Map_Tails_076A
	ptr	Map_Tails_0774
	ptr	Map_Tails_077E
	ptr	Map_Tails_0788
	ptr	Map_Tails_0792
	ptr	Map_Tails_079C
	ptr	Map_Tails_07A6
	ptr	Map_Tails_07B0
	ptr	Map_Tails_07BA
	ptr	Map_Tails_07C4
	ptr	Map_Tails_07CE
	ptr	Map_Tails_07E0
	ptr	Map_Tails_07F2
	ptr	Map_Tails_07FC
	ptr	Map_Tails_0806
	ptr	Map_Tails_0810
	ptr	Map_Tails_0822
	ptr	Map_Tails_0834
	ptr	Map_Tails_083E
	ptr	Map_Tails_0848
	ptr	Map_Tails_0852
	ptr	Map_Tails_0864
	ptr	Map_Tails_0876
	ptr	Map_Tails_0888
	ptr	Map_Tails_089A
	ptr	Map_Tails_08AC
	ptr	Map_Tails_08BE
	ptr	Map_Tails_08D0
	ptr	Map_Tails_08E2
	ptr	Map_Tails_08EC
	ptr	Map_Tails_08FE
	ptr	Map_Tails_0910
	ptr	Map_Tails_0922
	ptr	Map_Tails_0934
	ptr	Map_Tails_0946
	ptr	Map_Tails_0958
	ptr	Map_Tails_096A
	ptr	Map_Tails_097C
	ptr	Map_Tails_0986
	ptr	Map_Tails_0990
	ptr	Map_Tails_09A2
	ptr	Map_Tails_09AC
	ptr	Map_Tails_09B6
	ptr	Map_Tails_09C0
	ptr	Map_Tails_09CA
	ptr	Map_Tails_09D4
	ptr	Map_Tails_09DE
	ptr	Map_Tails_09E8
	ptr	Map_Tails_09F2
	ptr	Map_Tails_0A04
	ptr	Map_Tails_0A16
	ptr	Map_Tails_0A20
	ptr	Map_Tails_0A2A
	ptr	Map_Tails_0A34
	ptr	Map_Tails_0A3E
	ptr	Map_Tails_0A50
	ptr	Map_Tails_0A62
	ptr	Map_Tails_0A6C
	ptr	Map_Tails_0A76
	ptr	Map_Tails_0A80

Map_Tails_0116:	spritemap
	endsprite

Map_Tails_0118:	spritemap
	piece	-$C, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$C, 0, 3, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_012A:	spritemap
	piece	-$C, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$C, 0, 3, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_013C:	spritemap
	piece	-$C, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$C, 0, 3, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_014E:	spritemap
	piece	-$C, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$C, 0, 3, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_0160:	spritemap
	piece	-$C, -$10, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_016A:	spritemap
	piece	-$C, -$10, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0174:	spritemap
	piece	-$C, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$C, 0, 3, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_0186:	spritemap
	piece	-$C, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$C, 0, 3, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_0198:	spritemap
	piece	-$14, -$10, 2, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_01A2:	spritemap
	piece	-$1C, -$10, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_01AC:	spritemap
	piece	-$1C, -$10, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_01B6:	spritemap
	piece	-$1C, 0, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_01C0:	spritemap
	piece	-$1C, -$10, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_01CA:	spritemap
	piece	-$10, -$14, 3, 2, 0, 0, 0, 0, 0
	piece	-$10, -4, 4, 2, 6, 0, 0, 0, 0
	piece	-$1E, -4, 3, 2, $E, 0, 0, 0, 0
	endsprite

Map_Tails_01E4:	spritemap
	piece	-$D, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$10, 0, 4, 2, 6, 0, 0, 0, 0
	piece	-$1C, 0, 3, 2, $E, 0, 0, 0, 0
	endsprite

Map_Tails_01FE:	spritemap
	piece	-8, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$D, 0, 3, 2, 6, 0, 0, 0, 0
	piece	-$1C, -3, 3, 2, $C, 0, 0, 0, 0
	endsprite

Map_Tails_0218:	spritemap
	piece	-$E, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$E, 0, 3, 2, 6, 0, 0, 0, 0
	piece	-$1A, -3, 3, 2, $C, 0, 0, 0, 0
	endsprite

Map_Tails_0232:	spritemap
	piece	-$10, -$14, 3, 2, 0, 0, 0, 0, 0
	piece	-$10, -4, 4, 2, 6, 0, 0, 0, 0
	piece	-$1E, -4, 3, 2, $E, 0, 0, 0, 0
	endsprite

Map_Tails_024C:	spritemap
	piece	-$D, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$10, 0, 4, 2, 6, 0, 0, 0, 0
	piece	-$1C, 0, 3, 2, $E, 0, 0, 0, 0
	endsprite

Map_Tails_0266:	spritemap
	piece	-8, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$F, 0, 3, 2, 6, 0, 0, 0, 0
	piece	-$1C, -3, 3, 2, $C, 0, 0, 0, 0
	endsprite

Map_Tails_0280:	spritemap
	piece	-$E, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$E, 0, 3, 2, 6, 0, 0, 0, 0
	piece	-$1A, -3, 3, 2, $C, 0, 0, 0, 0
	endsprite

Map_Tails_029A:	spritemap
	piece	-$D, -$1D, 1, 2, 0, 0, 0, 0, 0
	piece	-$14, -$D, 1, 4, 2, 0, 0, 0, 0
	piece	-$C, -$D, 3, 4, 6, 0, 0, 0, 0
	endsprite

Map_Tails_02B4:	spritemap
	piece	-$A, -$19, 2, 2, 0, 0, 0, 0, 0
	piece	-$12, -9, 3, 4, 4, 0, 0, 0, 0
	piece	6, -2, 2, 2, $10, 0, 0, 0, 0
	endsprite

Map_Tails_02CE:	spritemap
	piece	-8, -$1A, 2, 2, 0, 0, 0, 0, 0
	piece	-$11, -$A, 1, 4, 4, 0, 0, 0, 0
	piece	-9, -$A, 3, 4, 8, 0, 0, 0, 0
	endsprite

Map_Tails_02E8:	spritemap
	piece	-9, -$1D, 1, 2, 0, 0, 0, 0, 0
	piece	-$14, -$D, 4, 4, 2, 0, 0, 0, 0
	piece	$C, -5, 1, 2, $12, 0, 0, 0, 0
	endsprite

Map_Tails_0302:	spritemap
	piece	-$D, -$1D, 1, 2, 0, 0, 0, 0, 0
	piece	-$14, -$D, 1, 4, 2, 0, 0, 0, 0
	piece	-$C, -$D, 3, 4, 6, 0, 0, 0, 0
	endsprite

Map_Tails_031C:	spritemap
	piece	-$A, -$19, 2, 2, 0, 0, 0, 0, 0
	piece	-$12, -9, 3, 4, 4, 0, 0, 0, 0
	piece	6, -1, 2, 2, $10, 0, 0, 0, 0
	endsprite

Map_Tails_0336:	spritemap
	piece	-8, -$1A, 2, 2, 0, 0, 0, 0, 0
	piece	-$11, -$A, 1, 4, 4, 0, 0, 0, 0
	piece	-9, -$A, 3, 4, 8, 0, 0, 0, 0
	endsprite

Map_Tails_0350:	spritemap
	piece	-9, -$1D, 1, 2, 0, 0, 0, 0, 0
	piece	-$14, -$D, 4, 4, 2, 0, 0, 0, 0
	piece	$C, 3, 1, 2, $12, 0, 0, 0, 0
	endsprite

Map_Tails_036A:	spritemap
	piece	-$14, -$10, 2, 4, 0, 0, 0, 0, 0
	piece	-4, -$10, 2, 4, 8, 0, 0, 0, 0
	piece	-4, 6, 2, 4, $10, 0, 0, 0, 0
	endsprite

Map_Tails_0384:	spritemap
	piece	-$10, -$10, 2, 4, 0, 0, 0, 0, 0
	piece	0, -$10, 2, 4, 8, 0, 0, 0, 0
	piece	0, 4, 2, 4, $10, 0, 0, 0, 0
	endsprite

Map_Tails_039E:	spritemap
	piece	-$10, -$10, 2, 4, 0, 0, 0, 0, 0
	piece	0, -$10, 2, 4, 8, 0, 0, 0, 0
	piece	-3, 4, 2, 4, $10, 0, 0, 0, 0
	endsprite

Map_Tails_03B8:	spritemap
	piece	-$10, -$10, 2, 4, 0, 0, 0, 0, 0
	piece	0, -$10, 2, 4, 8, 0, 0, 0, 0
	piece	-3, 4, 2, 4, $10, 0, 0, 0, 0
	endsprite

Map_Tails_03D2:	spritemap
	piece	-$14, -$10, 2, 4, 0, 0, 0, 0, 0
	piece	-4, -$10, 2, 4, 8, 0, 0, 0, 0
	piece	-4, 6, 2, 4, $10, 0, 0, 0, 0
	endsprite

Map_Tails_03EC:	spritemap
	piece	-$10, -$10, 2, 4, 0, 0, 0, 0, 0
	piece	0, -$10, 2, 4, 8, 0, 0, 0, 0
	piece	0, 4, 2, 4, $10, 0, 0, 0, 0
	endsprite

Map_Tails_0406:	spritemap
	piece	-$10, -$10, 2, 4, 0, 0, 0, 0, 0
	piece	0, -$10, 2, 4, 8, 0, 0, 0, 0
	piece	-3, 4, 2, 4, $10, 0, 0, 0, 0
	endsprite

Map_Tails_0420:	spritemap
	piece	-$10, -$10, 2, 4, 0, 0, 0, 0, 0
	piece	0, -$10, 2, 4, 8, 0, 0, 0, 0
	piece	-3, 4, 2, 4, $10, 0, 0, 0, 0
	endsprite

Map_Tails_043A:	spritemap
	piece	-$15, 4, 1, 2, 0, 0, 0, 0, 0
	piece	-$D, -$C, 4, 4, 2, 0, 0, 0, 0
	endsprite

Map_Tails_044C:	spritemap
	piece	-8, -$1E, 2, 2, 0, 0, 0, 0, 0
	piece	-$10, -6, 1, 2, 4, 0, 0, 0, 0
	piece	-8, -$E, 4, 4, 6, 0, 0, 0, 0
	endsprite

Map_Tails_0466:	spritemap
	piece	-$14, -7, 1, 2, 0, 0, 0, 0, 0
	piece	-$C, -$F, 4, 4, 2, 0, 0, 0, 0
	endsprite

Map_Tails_0478:	spritemap
	piece	-$11, -4, 1, 2, 0, 0, 0, 0, 0
	piece	-1, -$1B, 1, 2, 2, 0, 0, 0, 0
	piece	-9, -$B, 4, 4, 4, 0, 0, 0, 0
	endsprite

Map_Tails_0492:	spritemap
	piece	-$15, 4, 1, 2, 0, 0, 0, 0, 0
	piece	-$D, -$C, 4, 4, 2, 0, 0, 0, 0
	endsprite

Map_Tails_04A4:	spritemap
	piece	0, -$1E, 1, 2, 0, 0, 0, 0, 0
	piece	-$10, -6, 1, 2, 2, 0, 0, 0, 0
	piece	-8, -$E, 4, 4, 4, 0, 0, 0, 0
	endsprite

Map_Tails_04BE:	spritemap
	piece	-$14, -7, 1, 2, 0, 0, 0, 0, 0
	piece	-$C, -$F, 4, 4, 2, 0, 0, 0, 0
	endsprite

Map_Tails_04D0:	spritemap
	piece	-1, -$1C, 1, 2, 0, 0, 0, 0, 0
	piece	-$11, -4, 1, 2, 2, 0, 0, 0, 0
	piece	-9, -$C, 4, 4, 4, 0, 0, 0, 0
	endsprite

Map_Tails_04EA:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	piece	-$16, -$10, 2, 4, $10, 0, 0, 0, 0
	endsprite

Map_Tails_04FC:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	piece	-$16, -8, 2, 2, $10, 0, 0, 0, 0
	endsprite

Map_Tails_050E:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	piece	-$16, -8, 2, 2, $10, 0, 0, 0, 0
	endsprite

Map_Tails_0520:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	piece	-$16, -8, 2, 2, $10, 0, 1, 0, 0
	endsprite

Map_Tails_0532:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	piece	-$16, -$10, 2, 4, $10, 0, 0, 0, 0
	endsprite

Map_Tails_0544:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	piece	-$18, -$10, 2, 4, $10, 1, 1, 0, 0
	endsprite

Map_Tails_0556:	spritemap
	piece	-6, -$1C, 1, 2, 0, 0, 0, 0, 0
	piece	-$10, -$C, 3, 4, 2, 0, 0, 0, 0
	piece	-$18, -2, 3, 4, $E, 0, 0, 0, 0
	endsprite

Map_Tails_0570:	spritemap
	piece	-8, -$1C, 2, 2, 0, 0, 0, 0, 0
	piece	-$10, -$C, 3, 4, 4, 0, 0, 0, 0
	piece	-$11, 4, 2, 2, $10, 0, 0, 0, 0
	endsprite

Map_Tails_058A:	spritemap
	piece	-6, -$1C, 1, 2, 0, 0, 0, 0, 0
	piece	-$10, -$C, 3, 4, 2, 0, 0, 0, 0
	piece	-$11, 4, 2, 2, $E, 0, 0, 0, 0
	endsprite

Map_Tails_05A4:	spritemap
	piece	-8, -$1C, 2, 2, 0, 0, 0, 0, 0
	piece	-$10, -$C, 3, 4, 4, 0, 0, 0, 0
	piece	-$11, 4, 2, 2, $10, 0, 0, 0, 0
	endsprite

Map_Tails_05BE:	spritemap
	piece	-6, -$1C, 1, 2, 0, 0, 0, 0, 0
	piece	-$10, -$C, 3, 4, 2, 0, 0, 0, 0
	piece	-$16, -4, 3, 4, $E, 0, 0, 0, 0
	endsprite

Map_Tails_05D8:	spritemap
	piece	-8, -$1C, 2, 2, 0, 0, 0, 0, 0
	piece	-$10, -$C, 3, 4, 4, 0, 0, 0, 0
	piece	-$16, -9, 3, 4, $10, 1, 1, 0, 0
	endsprite

Map_Tails_05F2:	spritemap
	piece	-$10, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$10, 0, 4, 2, 6, 0, 0, 0, 0
	piece	-$10, 6, 4, 2, $E, 0, 0, 0, 0
	endsprite

Map_Tails_060C:	spritemap
	piece	-$10, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$10, 0, 4, 2, 6, 0, 0, 0, 0
	piece	-8, 6, 2, 2, $E, 0, 0, 0, 0
	endsprite

Map_Tails_0626:	spritemap
	piece	-$10, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$10, 0, 4, 2, 6, 0, 0, 0, 0
	piece	-8, 6, 2, 2, $E, 0, 0, 0, 0
	endsprite

Map_Tails_0640:	spritemap
	piece	-$10, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$10, 0, 4, 2, 6, 0, 0, 0, 0
	piece	-$10, 6, 4, 2, $E, 0, 0, 0, 0
	endsprite

Map_Tails_065A:	spritemap
	piece	-$10, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$10, 0, 4, 2, 6, 0, 0, 0, 0
	piece	-$10, 8, 4, 2, $E, 1, 1, 0, 0
	endsprite

Map_Tails_0674:	spritemap
	piece	-$14, -8, 3, 4, 0, 0, 0, 0, 0
	piece	4, -8, 2, 2, $C, 0, 0, 0, 0
	piece	0, -2, 3, 4, $10, 1, 0, 0, 0
	endsprite

Map_Tails_068E:	spritemap
	piece	-$14, -8, 3, 4, 0, 0, 0, 0, 0
	piece	4, -8, 2, 2, $C, 0, 0, 0, 0
	piece	4, 1, 2, 2, $10, 1, 0, 0, 0
	endsprite

Map_Tails_06A8:	spritemap
	piece	-$14, -8, 3, 4, 0, 0, 0, 0, 0
	piece	4, -8, 2, 2, $C, 0, 0, 0, 0
	piece	4, 1, 2, 2, $10, 1, 0, 0, 0
	endsprite

Map_Tails_06C2:	spritemap
	piece	-$14, -8, 3, 4, 0, 0, 0, 0, 0
	piece	4, -8, 2, 2, $C, 0, 0, 0, 0
	piece	4, 1, 2, 2, $10, 1, 0, 0, 0
	endsprite

Map_Tails_06DC:	spritemap
	piece	-$14, -8, 3, 4, 0, 0, 0, 0, 0
	piece	4, -8, 2, 2, $C, 0, 0, 0, 0
	piece	-2, -4, 3, 4, $10, 1, 0, 0, 0
	endsprite

Map_Tails_06F6:	spritemap
	piece	-$14, -8, 3, 4, 0, 0, 0, 0, 0
	piece	4, -8, 2, 2, $C, 0, 0, 0, 0
	piece	-1, -9, 3, 4, $10, 0, 1, 0, 0
	endsprite

Map_Tails_0710:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_071A:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0724:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_072E:	spritemap
	piece	-$24, -8, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0738:	spritemap
	piece	-$24, -8, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0742:	spritemap
	piece	-$24, -8, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_074C:	spritemap
	piece	-$24, -8, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0756:	spritemap
	piece	-$1F, 7, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0760:	spritemap
	piece	-$1F, 7, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_076A:	spritemap
	piece	-$1F, 7, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0774:	spritemap
	piece	-$1F, 7, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_077E:	spritemap
	piece	-8, $C, 2, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0788:	spritemap
	piece	-8, $C, 2, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0792:	spritemap
	piece	-8, $C, 2, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_079C:	spritemap
	piece	-8, $C, 2, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_07A6:	spritemap
	piece	7, 7, 2, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_07B0:	spritemap
	piece	7, 7, 2, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_07BA:	spritemap
	piece	7, 7, 2, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_07C4:	spritemap
	piece	7, 7, 2, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_07CE:	spritemap
	piece	-8, -$18, 2, 2, 0, 0, 0, 0, 0
	piece	-$10, -8, 3, 4, 4, 0, 0, 0, 0
	endsprite

Map_Tails_07E0:	spritemap
	piece	-8, -$18, 2, 2, 0, 0, 0, 0, 0
	piece	-$10, -8, 3, 4, 4, 0, 0, 0, 0
	endsprite

Map_Tails_07F2:	spritemap
	piece	-7, -$10, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_07FC:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0806:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0810:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	piece	-$16, -$1C, 4, 2, $10, 0, 0, 0, 0
	endsprite

Map_Tails_0822:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	piece	-$16, -$1C, 4, 2, $10, 1, 0, 0, 0
	endsprite

Map_Tails_0834:	spritemap
	piece	-$10, -$C, 4, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_083E:	spritemap
	piece	-$10, -$C, 4, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0848:	spritemap
	piece	-$10, -$C, 4, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0852:	spritemap
	piece	-8, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$10, 0, 4, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_0864:	spritemap
	piece	-8, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-8, 0, 3, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_0876:	spritemap
	piece	-8, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-8, 0, 3, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_0888:	spritemap
	piece	-8, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-8, 0, 3, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_089A:	spritemap
	piece	-$10, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$10, 0, 4, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_08AC:	spritemap
	piece	-$10, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$10, 0, 4, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_08BE:	spritemap
	piece	-$18, -8, 1, 2, 0, 0, 0, 0, 0
	piece	-$10, -$10, 3, 4, 2, 0, 0, 0, 0
	endsprite

Map_Tails_08D0:	spritemap
	piece	-$10, -$10, 3, 2, 0, 0, 0, 0, 0
	piece	-$18, 0, 4, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_08E2:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_08EC:	spritemap
	piece	-$14, -$10, 4, 4, 0, 0, 0, 0, 0
	piece	$C, -$10, 1, 4, $10, 0, 0, 0, 0
	endsprite

Map_Tails_08FE:	spritemap
	piece	-$14, -$10, 4, 4, 0, 0, 0, 0, 0
	piece	$C, -$10, 1, 4, $10, 0, 0, 0, 0
	endsprite

Map_Tails_0910:	spritemap
	piece	-$14, -8, 2, 2, 0, 0, 0, 0, 0
	piece	-4, -$10, 3, 4, 4, 0, 0, 0, 0
	endsprite

Map_Tails_0922:	spritemap
	piece	-$10, -$10, 2, 4, 0, 0, 0, 0, 0
	piece	0, -$10, 2, 4, 0, 1, 0, 0, 0
	endsprite

Map_Tails_0934:	spritemap
	piece	4, -8, 2, 2, 0, 1, 0, 0, 0
	piece	-$14, -$10, 3, 4, 4, 1, 0, 0, 0
	endsprite

Map_Tails_0946:	spritemap
	piece	-$10, -$10, 2, 4, 0, 0, 0, 0, 0
	piece	0, -8, 2, 2, 8, 0, 0, 0, 0
	endsprite

Map_Tails_0958:	spritemap
	piece	0, -$10, 2, 4, 0, 1, 0, 0, 0
	piece	-$10, -8, 2, 2, 8, 1, 0, 0, 0
	endsprite

Map_Tails_096A:	spritemap
	piece	-$14, -8, 2, 2, 0, 0, 0, 0, 0
	piece	-4, -$10, 3, 4, 4, 0, 0, 0, 0
	endsprite

Map_Tails_097C:	spritemap
	piece	-$10, -$10, 4, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0986:	spritemap
	piece	-$F, -$10, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0990:	spritemap
	piece	-8, -$1C, 2, 2, 0, 0, 0, 0, 0
	piece	-$F, -$C, 3, 4, 4, 0, 0, 0, 0
	endsprite

Map_Tails_09A2:	spritemap
	piece	-$E, -$E, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_09AC:	spritemap
	piece	-$10, -$C, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_09B6:	spritemap
	piece	-$E, -$D, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_09C0:	spritemap
	piece	-$E, -$E, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_09CA:	spritemap
	piece	-$E, -$E, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_09D4:	spritemap
	piece	-$F, -$10, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_09DE:	spritemap
	piece	-$10, -$13, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_09E8:	spritemap
	piece	-$F, -$14, 3, 4, 0, 0, 0, 0, 0
	endsprite

Map_Tails_09F2:	spritemap
	piece	-$D, -$C, 3, 2, 0, 0, 0, 0, 0
	piece	-$D, 4, 2, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_0A04:	spritemap
	piece	-$10, -$E, 2, 4, 0, 0, 0, 0, 0
	piece	0, -$14, 1, 4, 8, 0, 0, 0, 0
	endsprite

Map_Tails_0A16:	spritemap
	piece	-$20, -8, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0A20:	spritemap
	piece	-$20, -8, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0A2A:	spritemap
	piece	-$20, -8, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0A34:	spritemap
	piece	-$20, -8, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0A3E:	spritemap
	piece	-$C, -$18, 3, 2, 0, 0, 0, 0, 0
	piece	-$C, -8, 4, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_0A50:	spritemap
	piece	-$C, -$18, 3, 2, 0, 0, 0, 0, 0
	piece	-$C, -8, 4, 2, 6, 0, 0, 0, 0
	endsprite

Map_Tails_0A62:	spritemap
	piece	-$1D, -3, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0A6C:	spritemap
	piece	-$1D, -3, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0A76:	spritemap
	piece	-$1D, -3, 3, 2, 0, 0, 0, 0, 0
	endsprite

Map_Tails_0A80:	spritemap
	piece	-$1D, -3, 3, 2, 0, 0, 0, 0, 0
	endsprite

	even
