...
static int ReadByte(FILE *file, size_t *bytes_remaining)
{
...
void Encode(FILE *infile, FILE *outfile, size_t input_length)
{
	...
	do {
		...
		for (i = 0; i < last_match_length &&
				(c = ReadByte(infile, &input_length)) != EOF; i++) {
...
void Decode(FILE *infile, FILE *outfile, size_t input_length)	/* Just the reverse of Encode(). */
{
	int  i, j, k, r, c;
	unsigned int  flags;
	...
	for ( ; ; ) {
		if (((flags >>= 1) & 256) == 0) {
			if ((c = ReadByte(infile, &input_length)) == EOF) break;
		...
		if (flags & 1) {
			...
			if ((i = ReadByte(infile, &input_length)) == EOF) break;
			if ((j = ReadByte(infile, &input_length)) == EOF) break;
